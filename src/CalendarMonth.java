...
public class CalendarMonth implements Serializable{
    ...
    private List<ArrayList<String>> cm_events = new ArrayList<ArrayList<String>>(31);
    ...
    CalendarMonth(int year, int month){
        ...
        cm_month = month;
        for(int i =0;i<31;i++) {
            cm_events.add(i, new ArrayList<String>());
        }
    }
    ...
    public List<ArrayList<String>> getEventList(){
        return cm_events;
    }

    //特定の日のイベント(ArrayList)を返す
    public ArrayList<String> getDayEvent(int day){
        return cm_events.get(day-1);
    }

    //イベントを追加する 開始日:day_start 終了日:day_end
    public void addEvent(String new_event,int day_start, int day_end) {
        for(int i = day_start; i <= day_end; i++) {
            ArrayList<String> tmp_eventlist=  cm_events.get(i-1);
